<div>
    @if ($bitcoin_accounts->isNotEmpty())
        <div class="mt-6">
            <h2 class="text-xl font-semibold">Total value</h2>
            <div class="text-2xl mb-1 text-green-500">
                <span
                    class="currency-usd">{{ round($bitcoin_accounts->sum(fn($asset) => $asset->balance * $bitcoin_usd), 2) }}
                    $</span>
                <span
                    class="currency-eur">{{ round($bitcoin_accounts->sum(fn($asset) => $asset->balance * $bitcoin_eur), 2) }}
                    €</span>
            </div>
        </div>
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                @foreach ($bitcoin_accounts as $asset)
                    <div
                        class="flex items
                    -end justify-between rounded-lg border border-gray-100 bg-white p-6">
                        ...
                        -center gap-4">
                            ...
                            <div>
                                ...
                                <p class="text-2xl font-medium text-gray-900">
                                    ...
                                    <span
                                        class="currency-usd">{{ round($asset->balance * $bitcoin_usd, 2) }}
                                        $</span>
                                    <span
                                        class="currency-eur">{{ round($asset->balance * $bitcoin_eur, 2) }}
                                        €</span>
                                </p>
                            </div>
                        </div>
                        ...
                        <a href="{{ route('edit_bitcoin_account', $asset->id) }}" class="btn btn-primary">Edit</a>
                    </div>
                @endforeach
            @else
